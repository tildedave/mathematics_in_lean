...
theorem add_right_cancel {a b c : R} (h : a + b = c + b) : a = c := by
  rw [<- add_neg_cancel_right a b, <- add_neg_cancel_right c a, h]
  repeat rw [add_assoc, add_neg_cancel]
...
theorem neg_neg (a : R) : - -a = a := by
  apply neg_eq_of_add_eq_zero; rw [neg_add_cancel]
...
theorem mul_inv_rev (a b : G) : (a * b)⁻¹ = b⁻¹ * a⁻¹ := by
  rw [<- one_mul (b⁻¹ * a⁻¹), <- inv_mul_cancel (a * b), mul_assoc, mul_assoc, <- mul_assoc b _ _, mul_inv_cancel, one_mul, mul_inv_cancel, mul_one]
